...
```shell
  docker buildx build -t martinmsalas/hoppscotch-extension:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install martinmsalas/hoppscotch-extension:latest
```
...
```shell
  docker extension dev ui-source martinmsalas/hoppscotch-extension:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug martinmsalas/hoppscotch-extension:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset martinmsalas/hoppscotch-extension:latest
```
...
```shell
docker extension update martinmsalas/hoppscotch-extension:latest
```
...
```shell
docker extension rm martinmsalas/hoppscotch-extension:latest
```
